 ...
 const ProductCard  = (props :  any) => { 
    ...
    return <> {props.products.map((productData : any,index:number)=>{
        ...
        <Card.Title style={{fontSize:"15px",font:"roboto"}}>{`${productData.title} - ${productData.id}`}</Card.Title>
           <hr />
    <div>{`Brand - ${productData.handle}`}</div>
            <hr/>
        ...
        <Col  >
            <CardImg style={{padding:"1%",float:"right", height:"300px",width:"300px"}} src={productData.image ? productData.image.src : props.products[0].image.src} alt="Product Image" />
        </Col>
        </Row>
      </Card></Col></Row></>)
    })} </>
...
export const ProductData = () => {
   ...
   const [textValue,setTextValue] = useState("");
    ...
    />
        ...
        <ProductCard products={finalData} /></div></div>)
}
